...
import { T_extra } from "./translations";
...
// Merge extra languages
const T_all = { ...T, ...T_extra };
...
export default function App() {
  ...
  const t = (k: string) => T_all[lang]?.[k] ?? T_all.fr[k] ?? k;
        ...
        <header className="hdr">
          ...
          <div className="hdr-right">
            ...
            <div className="lang-wrap" onClick={e=>e.stopPropagation()}>
              <button className="lang-btn" onClick={()=>setShowLang(p=>!p)}>{T_all[lang]?.flag||"🌐"} {T_all[lang]?.code||"FR"} ▾</button>
              {showLang && (
                <div className="lang-drop">
                  {Object.entries(T_all).map(([l,v])=>(
                    <div key={l} className={"lang-opt"+(lang===l?" active":"")} onClick={()=>{setLangState(l);setShowLang(false);}}>
                      {v.flag} {l==="fr"?"Français":l==="en"?"English":l==="de"?"Deutsch":l==="hr"?"Hrvatski":l==="it"?"Italiano":l==="bg"?"Български":"Română":l==="es"?"Español":l==="pt"?"Português":l==="el"?"Hellas":l==="nl"?"Nederlands":"Polski"}
                    </div>
                  ))}
                </div>
              )}

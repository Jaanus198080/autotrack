...
const css = `
  ...
  *{margin:0;padding:0;box-sizing:border-box;}
  ...
  .step-dot{width:22px;height:22px;border-radius:50%;border:2px solid var(--border);background:var(--bg);display:flex;align-items:center;justify-content:center;font-size:10px;transition:all .3s;}
  .sd-done{border-color:var(--blue);background:var(--blue);color:#fff;}
  .sd-active{border-color:var(--blue);background:var(--bg);box-shadow:0 0 0 3px rgba(34,120,232,0.2);}
            ...
            <div className="res-wrap">
              ...
              <div className="top-card">
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14,gap:10}}>
                  <div>
                    <div style={{fontSize:11,fontWeight:700,letterSpacing:".18em",color:"var(--muted)",marginBottom:8,textTransform:"uppercase"}}>{trackId}</div>
                    <div style={{fontSize:18,fontWeight:700,color:"var(--text)",marginBottom:3}}>{trackData.client}</div>
                    <div style={{fontSize:13,color:"var(--muted)",marginBottom:2}}>{trackData.vehicle}{trackData.color&&trackData.color!=="—"?" · "+trackData.color:""}</div>
                    <div style={{fontSize:11,color:"var(--muted)"}}>{trackData.company}</div>
                  </div>
                  <div style={{textAlign:"right",flexShrink:0}}>
                    <div style={{fontSize:10,color:"var(--muted)",marginBottom:3,textTransform:"uppercase",letterSpacing:".08em"}}>Arrivée estimée</div>
                    <div style={{fontSize:16,fontWeight:700,color:"var(--text)"}}>{trackData.arr}</div>
                  </div>
                </div>
                <div style={{display:"flex",alignItems:"center",gap:8,background:"rgba(255,255,255,0.04)",borderRadius:8,padding:"10px 14px",marginBottom:12}}>
                  <span style={{fontSize:13,fontWeight:700,color:"var(--text)"}}>{trackData.fromCity||trackData.from?.split(",")[0]}</span>
                  <div style={{flex:1,height:1,background:"var(--border)"}}/>
                  <span style={{fontSize:16,color:"var(--muted)"}}>›</span>
                  <span style={{fontSize:13,fontWeight:700,color:"var(--text)"}}>{trackData.toCity||trackData.to?.split(",")[0]}</span>
                </div>
                        ...
                        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:8}}>
                          ...
                          <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6,flexShrink:0}}>
                            ...
                            <div style={{display:"flex",gap:5}}>
                              <button className="ref-btn" onClick={e=>{e.stopPropagation();selectTracking(id);}}>✏️</button>
                              <button className="del-btn" onClick={e=>{e.stopPropagation();setShowDeleteConfirm(id);}}>🗑️</button>
                            </div>
                          </div>
                        </div>
                      </div>
